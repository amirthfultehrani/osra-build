...
Textblock::Textblock( const int page_height, const Rectangle & block,
...
//  for( int i = 0; i < textlines(); ++i ) tlpv[i]->check_big_initials();
  ...
  if( textlines() > 2 )
    ...
    for( int i = 0, c = 0; i < lp->characters(); ++i )
      if( lp->character( i ).bottom() >= page_height - 1 &&
          2 * ++c >= lp->characters() )
    ...
    // page top == 0
    for( int i = 0, c = 0; i < lp->characters(); ++i )
      if( lp->character( i ).top() <= 1 && 2 * ++c >= lp->characters() )
...
const Textline & Textblock::textline( const int i ) const
  {
  if( i < 0 || i >= textlines() )
    throw Ocrad::Internal( "textline: index out of bounds." );
  return *tlpv[i];
  }
