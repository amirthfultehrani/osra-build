...
/* Recognize 1 blob characters without holes.
...
void Character::recognize110( const Charset & charset, const Rectangle & charbox )
  ...
  if( code == 'u' && f.lp.istpit() )		// Look for merged 'tr'
    {
  ...
  if( f.bp.minima() <= 2 &&
    ...
    if( code == 'n' )		// Look for '"' or merged 'rt' or 'fl'
      {
    ...
    else if( code == 'h' )		// Look for merged 'rf' or 'fi'
      {
    ...
    else if( code == 'k' )		// Look for merged 'rt'
      {
      if( 2 * f.lp[f.lp.pos(10)] > b.width() &&
          !f.rp.increasing( f.rp.pos( 75 ) ) &&
          set_merged_guess( 'r', b.hcenter(), 't', 0 ) ) return;
...
/* Recognize 1 blob characters with 1 hole.
   0469ADOPQRabdegopq#
*/
...
/* Recognize 1 blob characters with 2 holes.
   8BQg$&
*/
