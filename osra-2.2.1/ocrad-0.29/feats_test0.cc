...
/* Look for three black sections in column hcenter() ± n, then test whether
    ...
    if( 2 * vbar(0).width() <= b.width() && vbar(0).right() <= b.hcenter() )
      ...
      if( hbars() == 1 && Ocrad::similar( hbar(0).width(), b.width(), 10 ) &&
        ...
        if( hbar(0).bottom() >= botmin &&
            ...
            b.escape_right( b.vcenter(), b.hpos( 75 ) ) )
          return 'L';
...
/* Look for the nearest frontier in column hcenter(), then test whether gap
   is open downwards (except for 'x').
*/
...
/* Look for four black sections in column hcenter() ± 1, then test whether
...
/* Recognize single line, non-rectangular characters without holes.
   '/<>C[\^`c
*/
