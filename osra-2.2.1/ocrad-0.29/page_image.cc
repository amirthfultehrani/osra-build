...
void mirror_diagonal( std::vector< std::vector< uint8_t > > & data )
  ...
  const int cols = ( data.empty() ? 0 : data[0].size() );
  const int size = std::max( rows, cols );
  if( rows <= 0 || cols <= 0 ) return;

  if( rows < size )			// add rows
    {
    data.resize( size );
    for( int row = rows; row < size; ++row )
      data[row].resize( size );
    }
  else if( cols < size )		// add cols
  ...
  // swap the number of rows and cols
  if( cols < size ) data.resize( cols );
  else if( rows < size )
    for( int row = 0; row < cols; ++row )
      data[row].resize( rows );
  }

} // end namespace


// create a Page_image from an OCRAD_Pixmap
//
Page_image::Page_image( const OCRAD_Pixmap & image, const bool invert )
  {
  const int rows = image.height, cols = image.width;
  data.resize( rows );
...
Page_image::Page_image( const Page_image & source, const int scale )
  : maxval_( source.maxval_ ), threshold_( source.threshold_ )
  {
  if( scale < 2 || scale > source.width() || scale > source.height() )
    throw Ocrad::Internal( "bad parameter building a reduced Page_image." );

  const int scale2 = scale * scale;
  const int rows = source.height() / scale;
  const int cols = source.width() / scale;

  data.resize( rows );
  for( int row = 0; row < rows; ++row )
    {
    const int srow = ( row * scale ) + scale;
    data[row].reserve( cols );
    std::vector< uint8_t > & datarow = data[row];
...
void Page_image::threshold( const Rational & th )
  ...
  if( th >= 0 && th <= 1 )
    ...
    threshold_ = otsu_th( data, Rectangle( height(), width() ), maxval_ );
  }
...
void Page_image::threshold( const int th )
  ...
  else threshold_ = otsu_th( data, Rectangle( height(), width() ), maxval_ );
  }


bool Page_image::cut( const Rational ltwh[4] )
  {
  Rectangle re( height(), width() );

  const int l = absolute_pos( ltwh[0], 0, width() - 1 );
  if( l > re.left() ) { if( l < re.right() ) re.left( l ); else return false; }

  const int t = absolute_pos( ltwh[1], 0, height() - 1 );
  if( t > re.top() ) { if( t < re.bottom() ) re.top( t ); else return false; }

  const int r = l + absolute_pos( ltwh[2], 0, width() ) - 1;
  if( r < re.right() ) { if( r > re.left() ) re.right( r ); else return false; }

  const int b = t + absolute_pos( ltwh[3], 0, height() ) - 1;
  if( b < re.bottom() ) { if( b > re.top() ) re.bottom( b ); else return false; }

  if( re.width() < 3 || re.height() < 3 ) return false;

  // cutting is performed here
  if( re.bottom() < height() - 1 ) data.resize( re.bottom() + 1 );
  if( re.right() < width() - 1 )
    for( int row = height() - 1; row >= 0 ; --row )
      data[row].resize( re.right() + 1 );
  if( re.top() > 0 ) data.erase( data.begin(), data.begin() + re.top() );
  if( re.left() > 0 )
    for( int row = height() - 1; row >= 0 ; --row )
      data[row].erase( data[row].begin(), data[row].begin() + re.left() );
...
void Page_image::draw_mask( const Mask & m )
  {
  const int t = std::max( 0, m.top() );
  const int b = std::min( height() - 1, m.bottom() );
  ...
  for( int row = t + 1; row <= b; ++row )
    ...
    if( lprev >= 0 && lnew >= 0 )
      {
      int c1 = std::min( lprev, lnew );
      int c2 = std::min( width() - 1, std::max( lprev, lnew ) );
      for( int col = c1; col <= c2; ++col )
        set_bit( row, col, true );
      }
    if( rprev >= 0 && rnew >= 0 )
      {
      int c1 = std::min( rprev, rnew );
      int c2 = std::min( width() - 1, std::max( rprev, rnew ) );
      for( int col = c1; col <= c2; ++col )
        set_bit( row, col, true );
      }
...
void Page_image::draw_rectangle( const Rectangle & re )
  {
  const int l = std::max( 0, re.left() );
  const int t = std::max( 0, re.top() );
  const int r = std::min( width() - 1, re.right() );
  const int b = std::min( height() - 1, re.bottom() );
  if( l == re.left() )
...
void Page_image::draw_track( const Track & tr )
  ...
  int r = std::min( width() - 1, tr.right() );
  if( l == tr.left() )
    for( int row = tr.top( l ); row <= tr.bottom( l ); ++row )
      if( row >= 0 && row < width() ) set_bit( row, l, true );
  if( r == tr.right() )
    for( int row = tr.top( r ); row <= tr.bottom( r ); ++row )
      if( row >= 0 && row < height() ) set_bit( row, r, true );
  for( int col = l; col <= r; ++col )
    {
    int row = tr.top( col );
    if( row >= 0 && row < height() ) set_bit( row, col, true );
    row = tr.bottom( col );
    if( row >= 0 && row < height() ) set_bit( row, col, true );
    }
  }
...
bool Page_image::change_scale( int n )		// no change if n == ±1
  {
  if( n == 0 || n < -width() || n < -height() ) return false;
  if( n <= -2 ) { Page_image reduced( *this, -n ); *this = reduced; }
  else if( n >= 2 )
    {
    ...
    if( n >= 2 )	// scale 8-bit greyscale images keeping borders smooth
      {
      enlarge_n( data, n );
      if( maxval_ > 1 )
...
void Page_image::transform( const Transformation & t )
  ...
  switch( t.type() )
    ...
    case Transformation::mirror_d1:
      mirror_diagonal( data ); break;
    case Transformation::mirror_d2:
      mirror_diagonal( data );
      mirror_left_right( data ); mirror_top_bottom( data ); break;
    }
  }
