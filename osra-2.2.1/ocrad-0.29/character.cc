...
const Blob & Character::blob( const int i ) const
  {
  if( i < 0 || i >= blobs() )
    throw Ocrad::Internal( "const blob: index out of bounds" );
  return *blobpv[i];
  }
...
Blob & Character::blob( const int i )
  {
  if( i < 0 || i >= blobs() )
    throw Ocrad::Internal( "blob: index out of bounds" );
  return *blobpv[i];
  }
...
void Character::insert_guess( const int i, const int code, const int value )
  {
  if( i < 0 || i > guesses() )
    throw Ocrad::Internal( "insert_guess: index out of bounds" );
...
void Character::delete_guess( const int i )
  {
  if( i < 0 || i >= guesses() )
    throw Ocrad::Internal( "delete_guess: index out of bounds" );
  gv.erase( gv.begin() + i );
  }
...
void Character::swap_guesses( const int i, const int j )
  ...
  if( i < 0 || i >= guesses() || j < 0 || j >= guesses() )
    throw Ocrad::Internal( "swap_guesses: index out of bounds" );
...
const Character::Guess & Character::guess( const int i ) const
  {
  if( i < 0 || i >= guesses() )
    throw Ocrad::Internal( "guess: index out of bounds" );
  return gv[i];
  }
  ...
  for( int i = 0; i < guesses(); ++i )
    ...
    if( gv[i].code == '\t' )
      std::fprintf( control.outfile, "guess '\\t', confidence %d    ",
                             gv[i].value );
    else if( !control.utf8 || !gv[i].code )
      {
