...
void delete_hole( std::vector< Bitmap * > & holep_vector,
  ...
  if( i < 0 ) throw Ocrad::Internal( "delete_hole: lost hole." );
...
inline void join_holes( std::vector< Bitmap * > & holep_vector,
  ...
  if( i < 0 ) throw Ocrad::Internal( "join_holes: lost hole." );
  holep_vector.erase( holep_vector.begin() + i );
...
const Bitmap & Blob::hole( const int i ) const
  {
  if( i < 0 || i >= holes() )
    throw Ocrad::Internal( "hole: index out of bounds." );
  return *holepv[i];
  }
...
void Blob::fill_hole( const int i )
  {
  if( i < 0 || i >= holes() )
    throw Ocrad::Internal( "fill_hole: index out of bounds." );
  add_bitmap( *holepv[i] );
  delete holepv[i];
