...
void Textline::check_big_initials()
  {
...
Character & Textline::character( const int i ) const
  {
  if( i < 0 || i >= characters() )
    throw Ocrad::Internal( "character: index out of bounds." );
  return *cpv[i];
  }
...
bool Textline::is_key_character( const int i ) const
  ...
  if( i < big_initials_ || i >= characters() )
    throw Ocrad::Internal( "is_key_character: index out of bounds." );
...
void Textline::delete_character( const int i )
  {
  if( i < 0 || i >= characters() )
    throw Ocrad::Internal( "delete_character: index out of bounds." );
...
bool Textline::insert_space( const int i, const bool tab )
  {
  if( i <= 0 || i >= characters() )
    throw Ocrad::Internal( "insert_space: index out of bounds." );
  if( !height() )
    throw Ocrad::Internal( "insert_space: track not set yet." );
