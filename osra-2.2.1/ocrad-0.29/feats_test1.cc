...
/* Test whether the lower half of character is open to the left, to the right,
...
/* Test whether the upper half of character is open to the left, to the right,
   and/or to the bottom.
*/
int Features::test_6abd( const Charset & charset ) const
  ...
  if( 3 * h.width() < b.width() && ( bp.minima( b.height() / 4 ) != 1 ||
        tp.minima( h.vcenter() - b.top() ) != 1 ) ) return 0;
