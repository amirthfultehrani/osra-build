...
// create a Bitmap from part of another Bitmap
Bitmap::Bitmap( const Bitmap & source, const Rectangle & re )
  : Rectangle( re ), data( re.height() )
  {
  if( !source.includes( re ) )
    throw Ocrad::Internal( "bad parameter building a Bitmap from part of another Bitmap." );
...
// return false if bitmap is empty
//
...
/* Look for an inverted-U-shaped curve near the top, then test which of
...
/* Look for an U-shaped curve near the bottom, then test which of
