                   ...
                   unsigned int size)
{
  ...
  while (s != segments.end() && m != margins.end())
      ...
      if (ratio > MAX_RATIO || ratio == 0)
	{
      ...
      if (PARTS_IN_MARGIN*m->size() > MAX_MARGIN_SIZE)
	{
	  s = segments.erase(s);
	  m = margins.erase(m);
	  continue;
  ...
  if (set1.size() > 10 && (x2 - x1) > 3 && (y2 - y1) > 20 && (x2 - x1) < (y2 - y1))
    {
      ...
      //std::cout << x1 <<" " << y1 <<" "<<x2<<" "<<y2<<" " << i << std::endl;
  ...
  while (c != clusters.end())
    {
      ...
      square_area = (bottom - top+1) * (right - left+1);
      if (square_area != 0)
        ratio = 1. * area / square_area;

      if (ratio < MAX_RATIO && aspect > MIN_ASPECT && aspect < MAX_ASPECT)
        {
          box_t b1;
          boxes.push_back(b1);
...
std::pair<std::vector<unsigned char>, ColorGray> convert_image_to_mask(Image &image, double threshold, const ColorGray &bgColor, size_t w, size_t h)
{
  std::vector<unsigned char> out(w*h, 0);
  std::vector<double> shades;
  double bg_shade = bgColor.shade();
  std::vector<size_t> histogram_white(50, 0);
  std::vector<size_t> histogram_black(50, 0);
  for(size_t i = 0; i < h; ++i)
    {
      size_t length_white = 0;
      size_t length_black = 0;
      for (size_t j = 0; j < w; ++j)
	{
	  char r = 0;
	  ColorGray c = image.pixelColor(j, i);
	  double shade = c.shade();
	  if (fabs(shade - bg_shade) > threshold)
	    {
	      r = 1;
	      shades.push_back(shade);
	      if (length_white > 0 && length_white < histogram_white.size())
		++histogram_white[length_white];
	      length_white = 0;
	      ++length_black;
	    }
	  else
	    {
	      if (length_black > 0 && length_black < histogram_black.size())
		++histogram_black[length_black];
	      length_black = 0;
	      ++length_white;
	    }
	  out[i*w + j] = r;
  ...
  ColorGray frColor("black");
  if (!shades.empty())
    {
      std::sort(shades.begin(), shades.end());
      double median = shades[shades.size() / 2];
      frColor.shade(median);
    }
  auto max_white = std::max_element(histogram_white.begin(), histogram_white.end()); 
  size_t argmax_white = std::distance(histogram_white.begin(), max_white);
  auto max_black = std::max_element(histogram_black.begin(), histogram_black.end()); 
  size_t argmax_black = std::distance(histogram_black.begin(), max_black);
  if (argmax_white != 1 || argmax_black != 1)
    out.clear();
  return std::make_pair(out, frColor);
}

std::vector<unsigned char> dilation_dual_cache(const std::vector<unsigned char> &image, size_t w, size_t h, size_t n, size_t max_pixels)
{
  std::vector<unsigned char> out(w*h, 0);
  int before = (n - 1) / 2;
  ...
  for(; i < h; ++i)
      ...
      for (int jj = j - before; jj <= j + after; ++jj)
	{
      ...
      int cache = row_cache;
      out[i*w + j] = (cache >= max_pixels ? 1 : 0);
      ++j;
      for (; j < w; ++j)
	{
	  for (int ii = i - before; ii <= i + after; ++ii)
...
void save_transformed_image(const std::vector<unsigned char> &out, Image &image, size_t w, size_t h, double threshold, const ColorGray &bgColor, const ColorGray &frColor)
{
  double bg_shade = bgColor.shade();
  image.modifyImage();
  for(size_t i = 0; i < h; ++i)
    for (size_t j = 0; j < w; ++j)
      {
	size_t pos = j + i * w;
	unsigned char c = out[pos];
	ColorGray p = image.pixelColor(j, i);
	double shade = p.shade();
	if (c == 1 && fabs(shade - bg_shade) <= threshold )
	  {
	    image.pixelColor(j, i, frColor);
	  }
	else if (c == 0 && fabs(shade - bg_shade) > threshold )
	  {
	    image.pixelColor(j, i, bgColor);
	  }
      }
}

bool morphology_transform(Image &image, double threshold, const ColorGray &bgColor)
{
  size_t width = image.columns();
  size_t height = image.rows();
  std::vector<unsigned char> out;
  ColorGray frColor;
  std::tie(out, frColor) = convert_image_to_mask(image, threshold, bgColor, width, height);
  if (out.empty())
    return false;
  out = dilation_dual_cache(out, width, height, 4, 2); 
  out = dilation_dual_cache(out, width, height, 3, 5);
  save_transformed_image(out, image, width, height, threshold, bgColor, frColor);
  return true;
}
