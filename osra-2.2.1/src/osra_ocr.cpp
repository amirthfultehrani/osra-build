...
std::pair<size_t, int> ocr_page(const Image &image, double threshold, const ColorGray &bgColor)
{
  std::stringstream ss;
  std::vector<int> font_height;
  ...
  for (int chunk = 0; chunk < divide; ++chunk)
	...
	break;
      size_t tmp_height = std::min(height - chunk * height_offset, height_offset);
      ...
      unsigned char *pixmap = (unsigned char *) malloc(width * tmp_height);
      for (size_t i = 0; i < tmp_height; i++)
	for (size_t j = 0; j < width; j++)
	  pixmap[i * width + j] = (unsigned char) (255 - 255 * get_pixel(image, bgColor, j, chunk * height_offset + i, threshold));
  
      job1.src.p.x = width;
      job1.src.p.y = tmp_height;
      job1.src.p.bpp = 1;
      job1.src.p.p = pixmap;
	...
	{
	  auto box = (struct box *)list_get_current(&(job->res.boxlist));
	  if (box->num != -1 && box->c != UNKNOWN  &&  box->c != 0)
	    {	
	      auto dy = box->y1 - box->y0 + 1;
	      const char *c = decode(box->c, job->cfg.out_format);
	      if (c[0] != '_' && c[0] != ' ' && c[0] != '\n')
		font_height.push_back(dy);	  
	    }
  ...
  int median = 0;
  if (!font_height.empty())
    {
      std::sort(font_height.begin(), font_height.end());
      median = font_height[font_height.size() / 2];
    }
  ...
  return std::make_pair(str.size(), median);
}
