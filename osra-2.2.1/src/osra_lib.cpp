...
void set_select_resolution(std::vector<std::pair<int, eProcessing>>  &select_processing, int input_resolution, int font_height)
{
   if (input_resolution == 0)
    {
      if (font_height < MIN_FONT_HEIGHT)
	{
	  select_processing.resize(5);
	  select_processing[0] = std::make_pair(72, eProcessing::Normal);
	  select_processing[1] = std::make_pair(150, eProcessing::Normal);
	  select_processing[2] = std::make_pair(300, eProcessing::Normal);
	  select_processing[3] = std::make_pair(300, eProcessing::NoThinning);
	  select_processing[4] = std::make_pair(500, eProcessing::Scaling);	  
	}
      else if (font_height >= MIN_FONT_HEIGHT && font_height < 10)
	{	  
	  select_processing.resize(1);
	  select_processing[0] = std::make_pair(72, eProcessing::Normal);
	}
      else if (font_height >= 10 && font_height < 17)
	{	  
	  select_processing.resize(1);
	  select_processing[0] = std::make_pair(150, eProcessing::Normal);      
	}
      else if (font_height >= 17 && font_height < 23)
	{
	  select_processing.resize(2);
	  select_processing[0] = std::make_pair(300, eProcessing::Normal);
	  select_processing[1] = std::make_pair(300, eProcessing::NoThinning);      
	}
      else if (font_height >= 23 && font_height < 40)
	{	  
	  select_processing.resize(3);
	  select_processing[0] = std::make_pair(300, eProcessing::Normal);
	  select_processing[1] = std::make_pair(300, eProcessing::NoThinning);
	  select_processing[2] = std::make_pair(500, eProcessing::Scaling);     
	}
      else if (font_height >= 40)
	{
	  select_processing.resize(1);
	  select_processing[0] = std::make_pair(500, eProcessing::Scaling);
	}    
    }
   else if (input_resolution < 300)
    {
      select_processing.resize(1);
      select_processing[0] = std::make_pair(input_resolution, eProcessing::Normal);      
    }
  else if (input_resolution == 300)
    {
      select_processing.resize(2);
      select_processing[0] = std::make_pair(input_resolution, eProcessing::Normal);
      select_processing[1] = std::make_pair(input_resolution, eProcessing::NoThinning);      
    }
  else
    {      
      select_processing.resize(1);
      select_processing[0] = std::make_pair(input_resolution, eProcessing::Scaling);      
    }
}
...
// Loading the program data files into maps:
  if (!((spelling_file.length() != 0 && load_config_map(spelling_file, spelling))
    ...
    {
      std::cout << "Cannot open " << SPELLING_TXT << " file (tried locations \"" << DATA_DIR << "\", \"" << osra_dir
  ...
  if (!((superatom_file.length() != 0 && load_config_map(superatom_file, superatom))
    ...
    {
      std::cout << "Cannot open " << SUPERATOM_TXT << " file (tried locations \"" << DATA_DIR << "\", \"" << osra_dir
...
void create_thick_box(Image &orig_box,Image &thick_box,int &width,int &height,int &resolution,int &working_resolution,double &box_scale,
                      ColorGray bgColor, double THRESHOLD_BOND, eProcessing processing, bool &thick, bool jaggy, int font_height, bool verbose)
{
  if (resolution >= 300)
    {
      int max_hist;
      double nf45;
      double nf = noise_factor(orig_box, width, height, bgColor, THRESHOLD_BOND, resolution, max_hist, nf45);

      if (processing == eProcessing::NoThinning) 
	{
      ...
      if (processing == eProcessing::Scaling)
        {
	  if (max_hist >= 6)
            {
              int new_resolution = max_hist * 300 / 4;
	      if (verbose)
		std::cout << "Resolution guess by line thickness " << new_resolution << std::endl;
	    ...
	    {
	      resolution = static_cast<int> (300 * (static_cast<double>(font_height) / 20));
	      if (verbose)
		std::cout << "Resolution guess by font height " << resolution << std::endl;
            ...
            {
	      if (verbose)
		std::cout << "Default resolution guess " << resolution << std::endl;
              int percent = (100 * 300) / resolution;
              std::ostringstream scale;
              scale << percent << "%";
              orig_box.scale(scale.str());
      ...
      else if (nf > 0.5 && nf < 1. && max_hist <= 6)
	{
	  try
	    {
...
void split_fragments_and_assemble_structure_record(
    ...
    std::vector<std::string> &array_of_structures,
    std::vector<double> &array_of_avg_bonds,
    std::vector<double> &array_of_ind_conf,
    std::vector<Image> &array_of_images,
    std::vector<box_t> &array_of_boxes,
    int n_recognized_chars,
    bool show_learning,
    bool verbose,
  ...
  if (real_atoms > MIN_A_COUNT && real_atoms < MAX_A_COUNT && real_bonds < MAX_B_COUNT && bond_max_type>0 && bond_max_type<4)
    {
	  ...
	  if (verbose)
		...
		output_format = SUBSTITUTE_REACTION_FORMAT;
	      ...
	      remove_bracket_atoms(frag_atom, n_atom, frag_bond, n_bond, brackets, thickness, boxes[k], box_scale, real_font_width, real_font_height, bracket_boxes);
	      remove_zero_bonds(frag_bond, n_bond, frag_atom);
	      flatten_bonds(frag_bond, n_bond, frag_atom, 2*thickness);

	      assign_labels_to_brackets(bracket_boxes, label, n_label, letters, n_letters, real_font_width, real_font_height);
	      collapse_atoms(frag_atom, frag_bond, n_atom, n_bond, 3);

	      remove_zero_bonds(frag_bond, n_bond, frag_atom);
	      flatten_bonds(frag_bond, n_bond, frag_atom, 5);
	      remove_zero_bonds(frag_bond, n_bond, frag_atom);
		
	      fix_superatoms_which_are_metals(frag_atom, n_atom, frag_bond, n_bond, superatom);
              std::string structure =
                get_formatted_structure(frag_atom, frag_bond, n_bond, output_format, embedded_format,
                                        ...
                                        show_coordinates ? &coordinate_box : NULL, superatom, n_recognized_chars, show_learning, res_iter, verbose,
					bracket_boxes, ions, v3000);
		  ...
		  if ((molecule_statistics.rings56 > 0 || molecule_statistics.num_organic_non_carbon_atoms > 0)
		    ...
		    {
		      array_of_structures.push_back(structure); 
		      array_of_avg_bonds.push_back(page_scale * box_scale * avg_bond_length);
		      array_of_ind_conf.push_back(confidence);
		      array_of_boxes.push_back(rel_box);
...
Image process_pdf_page(poppler::document* doc,  poppler::page_renderer &r, int l, int resolution, bool verbose)
{
  poppler::page* p = doc->create_page(l);

  auto rect = p->page_rect();
  size_t width = std::ceil(rect.width() * resolution / 72);
  size_t height = std::ceil(rect.height() * resolution / 72);
  double lz = width *  height;
  int divide = std::ceil(lz / INT_MAX);
  if (verbose)
    std::cout << "Processing PDF page in " << divide << " chunk(s)" << std::endl;
  
  Image image(Geometry(width, height), "white");
  image.modifyImage();
  image.type(TrueColorType);

  size_t height_offset = height / divide;
  for (int chunk = 0; chunk < divide; ++chunk)
    {
      if (height <= chunk * height_offset)
	break;
      poppler::image im = r.render_page(p, resolution, resolution, 0, chunk * height_offset, width, std::min(height - chunk * height_offset, height_offset));
      ...
      for (int row = 0; row < im.height(); ++row)
	{
	  for (int col = 0; col < im.width(); ++col)
		...
		case poppler::image::format_argb32 :
	      ...
	      if (r >= 0 && g >= 0 && b >= 0 && col < width && row + chunk * height_offset < height)
		image.pixelColor(col, row + chunk * height_offset, ColorRGB(double(r) / 255, double(g) / 255, double(b) / 255));
	    }
	}
    }
  return image;
...
void __attribute__ ((constructor)) osra_init()
{
  ...
  if (global_init_state != 0)
    std::cout << "OpenBabel initialization failure." << std::endl;
#endif
...
)
{
  ...
  if (type.empty())
    {
#ifdef OSRA_LIB
      std::cout << "Cannot detect blob image type" << std::endl;
#else
  ...
  if (!output_file.empty())
    {
      ...
      if (outfile.bad() || !outfile.is_open())
        {
          std::cout << "Cannot open file \"" << output_file << "\" for output" << std::endl;
  ...
  if (!embedded_format.empty() && !(output_format == "sdf" && (embedded_format == "inchi" || embedded_format == "smi"
                                    || embedded_format == "can")))
    {
      std::cout << "Embedded format option is only possible if output format is SDF and option can have only inchi, smi, or can values." << std::endl;
  ...
  std::vector<std::vector<std::vector<std::vector<std::string> > > > array_of_structures_page(    // page-resulution-box-structures
      page, std::vector<std::vector<std::vector<std::string> > >(NUM_RESOLUTIONS));
  std::vector<std::vector<std::vector<std::vector<double> > > > array_of_avg_bonds_page(
      page, std::vector<std::vector<std::vector<double> > >(NUM_RESOLUTIONS));
  std::vector<std::vector<std::vector<std::vector<double> > > > array_of_ind_conf_page(
      page, std::vector<std::vector<std::vector<double> > >(NUM_RESOLUTIONS));
  std::vector<std::vector<std::vector<std::vector<Image> > > > array_of_images_page(
      page, std::vector<std::vector<std::vector<Image> > >(NUM_RESOLUTIONS));
  std::vector<std::vector<std::vector<std::vector<box_t> > > > array_of_boxes_page(
      page, std::vector<std::vector<std::vector<box_t> > >(NUM_RESOLUTIONS));
  std::vector<Image> page_images;
//#pragma omp parallel for default(shared) private(OCR_JOB,JOB)
  for (int l = 0; l < page; l++)
	...
	{
	  image = process_pdf_page(poppler_doc, poppler_renderer, l, pdf_input_resolution, verbose);
	}
      else
	{
#ifdef OSRA_LIB
...
//#pragma omp critical
      ...
      if (l == 0 && !preview.empty())
	{
      ...
      std::vector<std::vector<std::vector<std::string> > > &array_of_structures_res = array_of_structures_page[l]; // resolution-box-structure
      std::vector<std::vector<std::vector<double> > > &array_of_avg_bonds_res = array_of_avg_bonds_page[l];
      std::vector<std::vector<std::vector<double> > > &array_of_ind_conf_res = array_of_ind_conf_page[l];
      std::vector<std::vector<std::vector<Image> > > &array_of_images_res = array_of_images_page[l];
      std::vector<std::vector<std::vector<box_t> > > &array_of_boxes_res = array_of_boxes_page[l];          

      ColorGray bgColor = getBgColor(image);
      bool morphology_transformed = morphology_transform(image, 0.1, bgColor); 
      size_t max_length;
      int font_height;
      std::tie(max_length, font_height) = ocr_page(image, 0.1, bgColor);
      if (rotate != 0)
        {
      ...
      else 
	{
	  ...
	  for (int a = 90; a < 360; a += 90)
		...
		continue;
	      size_t tmp_length;
	      int tmp_font_height;
	      std::tie(tmp_length, tmp_font_height) = ocr_page(image, 0.1, bgColor);
	...
	std::cout << "Font height: " << font_height  << std::endl;
      std::vector<std::pair<int, eProcessing>> select_processing;
      set_select_resolution(select_processing, input_resolution, font_height);
      ...
      for (int res_iter = 0; res_iter < NUM_RESOLUTIONS; res_iter++)
        {
	  std::vector<std::vector<std::string> >  &array_of_structures_box = array_of_structures_res[res_iter]; // box-structure
	  std::vector<std::vector<double> >  &array_of_avg_bonds_box = array_of_avg_bonds_res[res_iter];
	  std::vector<std::vector<double> >  &array_of_ind_conf_box = array_of_ind_conf_res[res_iter];
	  std::vector<std::vector<Image> >  &array_of_images_box = array_of_images_res[res_iter];
	  std::vector<std::vector<box_t> > &array_of_boxes_box = array_of_boxes_res[res_iter];
	  array_of_structures_box.resize(n_boxes);
	  array_of_avg_bonds_box.resize(n_boxes);
	  array_of_ind_conf_box.resize(n_boxes);
	  array_of_images_box.resize(n_boxes);
	  array_of_boxes_box.resize(n_boxes);
	}
      page_images.push_back(image);      
      for (int res_iter = 0; res_iter < select_processing.size(); res_iter++)
        {
	  std::vector<std::vector<std::string> >  &array_of_structures_box = array_of_structures_res[res_iter]; // box-structure
	  std::vector<std::vector<double> >  &array_of_avg_bonds_box = array_of_avg_bonds_res[res_iter];
	  std::vector<std::vector<double> >  &array_of_ind_conf_box = array_of_ind_conf_res[res_iter];
	  ...
	  std::tie(resolution, processing)= select_processing[res_iter];
		...
		//auto before = std::chrono::system_clock::now().time_since_epoch();
                ...
                Image thick_box;
                create_thick_box(orig_box,thick_box,width,height,resolution,working_resolution,box_scale,bgColor,THRESHOLD_BOND,processing,thick,jaggy,font_height, verbose);
		...
		//if (l == 0 && res_iter == 1 && k == 0)  debug_image(orig_box, atom, n_atom, bond, n_bond, "tmp.png");
		if (n_atom >= MAX_ATOMS)
		  {
		    if (st != NULL)
		...
		find_arrow_bonds(bond, n_bond, atom, thick_box, bgColor, THRESHOLD_BOND);
		...
		std::vector<std::string> &array_of_structures = array_of_structures_box[k];
		std::vector<double> &array_of_avg_bonds = array_of_avg_bonds_box[k];
		std::vector<double> &array_of_ind_conf = array_of_ind_conf_box[k];
		std::vector<Image> &array_of_images = array_of_images_box[k];
		std::vector<box_t> &array_of_boxes = array_of_boxes_box[k];
                split_fragments_and_assemble_structure_record(atom,n_atom,bond,n_bond,boxes,
							      ...
							      array_of_avg_bonds,array_of_ind_conf,array_of_images,array_of_boxes,
							      num_recognized_chars,show_learning,verbose, brackets, ions, v3000, label, n_label, letters, n_letters);

                if (st != NULL)
                  potrace_state_free(st);
		//after = std::chrono::system_clock::now().time_since_epoch();
		//duration = std::chrono::duration_cast<std::chrono::nanoseconds>(after - before).count();      
	      }
        }

      // page-resulution-box-structures
      for (int k = 0; k < n_boxes; ++k)
	{
	  double max_conf = -FLT_MAX;
	  int max_res = -1;      
	  for (int res_iter = 0; res_iter < NUM_RESOLUTIONS; ++res_iter)
	    if (!array_of_ind_conf_page[l][res_iter][k].empty())
	    {
	      double confidence = std::reduce(array_of_ind_conf_page[l][res_iter][k].begin(), array_of_ind_conf_page[l][res_iter][k].end());
	      confidence /= array_of_ind_conf_page[l][res_iter][k].size();
	  ...
	  for (int j = 0; j < NUM_RESOLUTIONS; j++)
	    {
	      if (!show_learning && j != max_res)
		{
		  array_of_structures_page[l][j][k].clear();
		  if (!output_image_file_prefix.empty())
		    array_of_images_page[l][j][k].clear();
		  array_of_avg_bonds_page[l][j][k].clear();
		  array_of_ind_conf_page[l][j][k].clear();
		  array_of_boxes_page[l][j][k].clear();
		}
	    }
	}
    }
  ...
  int k_index = 0;
  int i_index = 0;
  static std::atomic<int> image_count; 

  for (int l = 0; l < page; l++)
    {
      std::vector<box_t> boxes_page;
      std::vector<std::string> structures_page;
   
      for (int j = 0; j < NUM_RESOLUTIONS; ++j)
	for (int k = 0; k < array_of_structures_page[l][j].size(); k++)
	  for (unsigned int i = 0; i < array_of_structures_page[l][j][k].size(); i++)
	    //if (best_bond == 0 || (array_of_avg_bonds_page[l][j][k][i] > best_bond/2 && array_of_avg_bonds_page[l][j][k][i] < 2*best_bond))
	      {
		if (array_of_ind_conf_page[l][j][k][i] > max_confidence)
		  {
		    max_confidence = array_of_ind_conf_page[l][j][k][i];
		    l_index = l;
		    j_index = j;
		    k_index = k;
		    i_index = i;
		...
		if (output_format != "mol" && !is_reaction)
		  {
		    out_stream << array_of_structures_page[l][j][k][i];
			...
			if (fname.str() != "")
			  {
			    Image &tmp = array_of_images_page[l][j][k][i];
			    if (resize != "")
			      {
				tmp.scale(resize);
			      }
		...
		boxes_page.push_back(array_of_boxes_page[l][j][k][i]);
		structures_page.push_back(array_of_structures_page[l][j][k][i]);	
	      }
      ...
      if (is_reaction && !arrows[l].empty())
	{
	  ...
	  arrange_reactions(arrows[l], boxes_page, pluses[l], reactions, rbox, structures_page, output_format); 
  ...
  if (output_format == "mol")
    {
      if (k_index < array_of_structures_page[l_index][j_index].size() && i_index < array_of_structures_page[l_index][j_index][k_index].size())
	{
	  out_stream << array_of_structures_page[l_index][j_index][k_index][i_index];     
	  if (!output_image_file_prefix.empty())
	    {
	      std::ostringstream fname;
	      fname << output_image_file_prefix  << image_count << ".png";
	      ...
	      if (fname.str() != "")
		{
		  Image &tmp = array_of_images_page[l_index][j_index][k_index][i_index];
		  if (resize != "")
		    {
		      tmp.scale(resize);
		    }
