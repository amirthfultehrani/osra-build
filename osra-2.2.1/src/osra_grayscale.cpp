...
void otsu_find_peaks(const std::vector<size_t> &h, int num_bins, int &peak1, int &peak2, int &max1, int &max2)
{
// Otsu Algorithm, from http://habrahabr.ru/blogs/algorithm/112079/
  size_t m = 0;
  size_t n = 0;
  double maxSigma = -1;
  size_t min_t = num_bins;
  size_t alpha1 = 0;
  size_t beta1 = 0;
...
Image adaptive_otsu(const Image &image, int window)
{
  ...
  std::vector<size_t> h(num_bins,0);
  std::vector<size_t> h0(num_bins,0);
  ColorGray g;
