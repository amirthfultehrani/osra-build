...
void mark_aromatic_bonds(const potrace_path_t *p, std::vector<bond_t> &bond, int n_bond,  std::vector<atom_t> &atom, int n_atom, double avg)
{
  std::vector<int> vert;
  double circum = 0;
  for (int i = 0; i < n_bond; i++)
  ...
  for (int i = 0; i < n_atom; i++)
    if (atom[i].exists && atom[i].curve == p)
      vert.push_back(i);
  if (vert.size() > 4)
    {
          ...
          if (child != NULL && child->sign == int('+'))
            {
              ...
              if (gchild != NULL && gchild->sign == int('-'))
                {                  
		  mark_aromatic_bonds(p, bond, n_bond,  atom,  n_atom,  avg);
                }
            }
        }
      p = p->next;
      ...
      if (p1->sign == int('+') && detect_curve(bond, n_bond, p1))
        {
          ...
          if (child != NULL && child->sign == int('-'))
            {
              mark_aromatic_bonds(p1, bond, n_bond,  atom,  n_atom,  avg);
            } 
        }
      p1 = p1->next;
    }
...
{
  int intersect = std::max(0, std::min(a.x2, b.x2) - std::max(a.x1, b.x1)) * std::max(0, std::min(a.y2, b.y2) - std::max(a.y1, b.y1));
  int un = (a.x2 - a.x1) * (a.y2 - a.y1) + (b.x2 - b.x1) * (b.y2 - b.y1) - intersect;
  double overlap = static_cast<double>(intersect) / un;
  return overlap > 0.5;   
}

std::vector<std::stack<std::pair<int, int> > > in_ring(const std::vector<std::vector<std::pair<int,int> > > &adjacency,
	 const int n_bond, const int start, const int max_depth = 6)
{
  ...
  std::vector<std::stack<std::pair<int, int> > > rings;
...
{
  return (ring.size() == 5 || ring.size() == 6);
  /* if (ring.size() != 6)
    return false;
...
void remove_bracket_atoms(std::vector<atom_t> &atom, int n_atom,
			  std::vector<bond_t> &bond, int n_bond,
                          ...
                          double thickness, const box_t &box, double box_scale,
                          ...
                          std::vector<bracket_t> &reduced_bracket_boxes)
{
  ...
  std::vector<std::vector<bool> > is_bond(n_atom, std::vector<bool>(n_atom, false));
  for (int i = 0; i < n_bond; ++i)
    if (bond[i].exists)
    {
      bond_type[bond[i].a] = std::max(bond_type[bond[i].a], bond[i].type);
      bond_type[bond[i].b] = std::max(bond_type[bond[i].b], bond[i].type);
      if (bond[i].arom)
	{
      ...
      is_bond[bond[i].a][bond[i].b] = true;
      is_bond[bond[i].b][bond[i].a] = true;
    }
  ...
  for (int i = 0; i < n_atom; i++)
    if (atom[i].exists  && (atom[i].label == " " || atom[i].label.empty()) && bond_type[i] == 1)
      {
	auto rings = in_ring(adjacency, n_bond, i);
	if (std::any_of(rings.begin(), rings.end(), [&atom] (std::stack<std::pair<int, int> >  &ring) {return real_ring(ring, atom);}))
	  continue;
	bool found = false;
	double x =   (double)  box_scale * atom[i].x + box.x1 -  FRAME;
	double y =   (double)  box_scale * atom[i].y + box.y1 -  FRAME;
	for (std::set<std::pair<int, int> >::const_iterator j = brackets.begin(); j != brackets.end(); ++j)
	  if (j->first >= box.x1 && j->first <= box.x2 && j->second >= box.y1 && j->second <= box.y2 &&
	      distance(x, y, j->first, j->second) < thickness)
	    {
	      found = true;
	    }
	if (found)
  ...
  for (auto &i :  candidates)
	...
	if (j >= i && is_bond[i][j])
	  {
	     double x1 =   (double)  box_scale * atom[i].x + box.x1 -  FRAME;
	     double y1 =   (double)  box_scale * atom[i].y + box.y1 -  FRAME;
	     double x2 =   (double)  box_scale * atom[j].x + box.x1 -  FRAME;
	     double y2 =   (double)  box_scale * atom[j].y + box.y1 -  FRAME;
	     if (fabs(y1 - y2) < thickness && fabs(x1 - x2) >  2 * scaled_font_width)
	       {
		 i = -1;
		 j = -1;
		 break;
  ...
  for (int i = 0; i < horizontals.size(); i++)
	...
	if ( fabs(horizontals[i].first.x - horizontals[j].first.x) < thickness &&
	     ...
	     fabs(horizontals[i].second.y - horizontals[j].second.y) > 1.5 * scaled_font_height)
	  {
	    ...
	    if (b.x1 >= box.x1 && b.x1 <= box.x2 &&
		b.x2 >= box.x1 && b.x2 <= box.x2 &&
		b.y1 >= box.y1 && b.y1 <= box.y2 &&
		b.y2 >= box.y1 && b.y2 <= box.y2)
	      {
  ...
  for (int i = 0; i < bracket_boxes.size(); i++)
      ...
      if (bracket_boxes[i].x1 != -1)
	{
	  ...
	  b.x1 = int(double(b.x1 + FRAME - box.x1) / box_scale);
	  b.y1 = int(double(b.y1 + FRAME - box.y1) / box_scale);
	  b.x2 = int(double(b.x2 + FRAME - box.x1) / box_scale);
	  b.y2 = int(double(b.y2 + FRAME - box.y1) / box_scale);
	  bracket_t bracket;
	  bracket.box = b;
  ...
  for (int i = 0; i < n_bond; ++i)
	...
	if (remove_atoms[bond[i].a] && !remove_atoms[bond[i].b])
	  remove_atoms[bond[i].a] = false;
	if (remove_atoms[bond[i].b] && !remove_atoms[bond[i].a])
	  remove_atoms[bond[i].b] = false;
      }

  for (const auto &brack : reduced_bracket_boxes)
    {
      const auto &b = brack.box;
      for (int i = 0; i < n_atom; ++i)
	if (atom[i].exists && !remove_atoms[i])
	  {
	    if ( (fabs(atom[i].x - b.x1) < thickness && fabs(atom[i].y - b.y1) < thickness) ||
		 (fabs(atom[i].x - b.x1) < thickness && fabs(atom[i].y - b.y2) < thickness) ||
		 (fabs(atom[i].x - b.x2) < thickness && fabs(atom[i].y - b.y1) < thickness) ||
		 (fabs(atom[i].x - b.x2) < thickness && fabs(atom[i].y - b.y2) < thickness))
	      {
		remove_atoms[i] = true;
	      }
	  }
  ...
  for (const auto &brack : reduced_bracket_boxes)
    {
      const auto &b = brack.box;
      for (int i = 0; i < n_atom; i++)
	if (atom[i].exists && atom[i].label == "Xx" &&
	    (fabs(atom[i].x - b.x1) < thickness || fabs(atom[i].x - b.x2) < thickness) &&
	    atom[i].y >= b.y1 && atom[i].y <= b.y2)
	  {
	    atom[i].label = " ";
	  }
    }
...
void fix_superatoms_which_are_metals(std::vector<atom_t> &atom, int n_atom, const std::vector<bond_t> &bond, int n_bond, const std::map<std::string, std::string> &superatom)
{
  for (int i = 0; i < n_atom; ++i)
    if (atom[i].exists)
      {
	std::map<std::string, std::string>::const_iterator it = superatom.find(atom[i].label);

	if (it != superatom.end() || atom[i].label == "Ir")
	  {
	    int n_neighbors = 0;
	    for (int j = 0; j < n_bond; ++j)
	      if (bond[j].exists && (bond[j].a == i || bond[j].b == i))
